...
def grade():
    ...
    with open(os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "Results", "AgentAnswers.csv"))) as fd:
    ...
    results = open(os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "Results", "ProblemResults.csv")), "w")
    results.write("Problem,Agent's Answer,Correct?,Correct Answer\n")

    setResults = open(os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "Results", "SetResults.csv")), "w")
    setResults.write("Set,Correct,Incorrect,Skipped\n")

    with open(os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "Problems", "ProblemSetList.txt"))) as fd0:
        for line0 in fd0:
            line0 = line0.rstrip()
            totals = {"Correct": 0, "Skipped": 0, "Incorrect": 0}
            with open(os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "Problems", line0, "ProblemList.txt"))) as fd1:
                for line1 in fd1:
                    line1 = line1.rstrip()
                    with open(os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "Problems", line0, line1, "ProblemAnswer.txt"))) as fd2:
                        truth = int(fd2.read())
